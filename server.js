...
app.post("/addcity",(req,res)=>{
  ...
  file.mv(`${process.cwd()}/client/public/city/${file.name}`)
   file.mv(`${process.cwd()}/client1/public/city/${file.name}`,(err)=>{
   ...
   data.bgimg=file.name
   data.lockdown=req.body.lock

  data.save().then(()=>{console.log("data is saved")}).catch((error)=>{console.log("data is not saved")})
    }
...
if(req.files==null){
  ...
  db1.findByIdAndUpdate({_id:req.body.id},{$set:{name:req.body.status,lockdown:req.body.status1,bgimg:req.files.image.name}}).then(()=>{res.send("update")}).catch(res=>{
    res.send("not update")
  })
}
 })
