...
export default function BasicTable() {
  ...
  useEffect(()=>{
  axios.get("http://localhost:5000/getdata").then(res=>{console.log(res.data);
        setdata(res.data)
      })
...
const ddelete=async (i)=>{
 await axios.post("http://localhost:5000/delete",{id:i}).then((res)=>{console.log(res.data);
      ...
      <Table className={classes.table} aria-label="simple table">
        ...
        <TableBody>
              ...
              <TableCell  component="th" scope="row">
               <img src={process.env.PUBLIC_URL+`/city/${i.bgimg}`} width="200px"/>
              </TableCell>
...
const Edit=()=>{
  ...
  const hsitory=useHistory()
    ...
    const handleChange = (event) => {
      ...
      useEffect(()=>{
          axios.post("http://localhost:5000/getid",{id:deleteid}).then((res)=>{
...
formData.append('id',id);
   ...
   axios.post("http://localhost:5000/updateeeee",formData,{headers: {
