...
export default function BasicTable() { 
  ...
  const [catogary, setcatogary] = useState([])
    useEffect( ()=>{
  axios.get("http://localhost:5000/admin/viewproducts").then(res=>{setpro(res.data)})
    },[pro])


    useEffect(()=>{
      axios.get("http://localhost:5000/admin/viewcatogary").then((res)=>setcatogary(res.data)).catch(()=>{console.log("error")})
      .catch(()=>{console.log("error")})

      
      axios.get("http://localhost:5000/admin/viewsubcatogary").then((res)=>{
    ...
    const remove= async(id)=>{
      ...
      if (window.confirm("Do you want to delete this item ?")) {
        ...
        await axios.post("http://localhost:5000/admin/removeproduct",{idd:id},{headers: {
    token:user.token
  }}).then(res=>{Setmsg(res.data);setOpen(true);setTimeout(()=>{setOpen(false)},2000)})

  await axios.get("http://localhost:5000/admin/viewproducts").then(res=>{
...
<FormControl fullWidth variant="outlined"  >
  ...
  {newarray1.map((i,v)=>
    <tr>
   <td><img src={process.env.PUBLIC_URL+`/product/${i.image}`} width="150px" height="120px"/></td>
    <td >
...
function Edit() {
 ...
 axios.post("http://localhost:5000/admin/updateproduct",{idd:productID},{headers: {
    token:user.token
  }}).then(res=>{
...
axios.post("http://localhost:5000/admin/updateproduct1",formData,{headers: {
